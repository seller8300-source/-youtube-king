...
[1] 아래 7개 공식 중 후보마다 '서로 다른 2개'를 섞어라. 3후보가 모두 다른 조합이어야 한다.
   ① 권위부여 — 전문가·경력·실적 ("20년 시공팀이", "현장 800곳이")
   ② 반박제거 — 의심·불안을 먼저 짚기 ("이런 분들은 사지 마세요", "효과 못 본 분만")
   ③ 통념반박 — 당연한 상식 뒤집기 ("비싼 돌이 오히려 손해", "에어컨이 더 덥게 한다")
   ④ 숫자제시 — 구체적 수치 (%, 배, 분, 초, 도, 원, 명, 가지)
   ⑤ 손실회피 — 모르면 손해 ("속으면 돈만 날립니다", "이거 모르면 호구")
   ⑥ 고객갈망 — 진짜 원하는 결과 욕망 (키·돈·고급·시간·인정)
   ⑦ 최상의이득 — 결과가 이뤄진 장면 ("손님이 감탄하는 매장")

[2] 아래는 '느낌·구조' 정답지다. (병원 예시지만 골격만 배우고, 우리 제품 업종 말투로 바꿔라)
   · "현직 의사가 폭로합니다" → [우리식] "20년 시공팀이 폭로합니다"
   · "받으면 호구되는 시술 5가지" → [우리식] "사면 호구되는 자재 5가지"
   · "의사는 여기 갑니다" → [우리식] "고수는 이거 씁니다"
   · "10명 중 8명이 모르는 진실" → "사장님 80%가 모르는 진실"
   · "이런 분들은 받지 마세요" → "이런 분들은 사지 마세요"
   · "광고에 속으면 돈만 날립니다" → "싼 거 속으면 돈만 날립니다"
   · "기미는 2가지만 보면 됩니다" → "자재는 2가지만 보면 됩니다"
   · "잘못된 오해와 진실" "주의해야 한다면 이것" "이런 곳에서 하면 큰일납니다" 같은 폭로/경고 골격도 활용.

[3] 표본 넓히기: 제품명 직접 노출 금지. 증상/제품 → 욕망 → 최상의 이득으로 끌어올려라.

[4] 매번 다른 결이 나오게, 공식 조합을 다양하게 바꿔라. 무난하면 실패다. 폭로·반전·숫자로 스크롤을 멈춰라.`,
...
7. 카피는 7개 공식(권위·반박제거·통념반박·숫자·손실회피·고객갈망·최상의이득)을 다양하게 조합해 써라. 무난하면 실패.
