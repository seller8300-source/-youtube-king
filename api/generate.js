...
두 종류로 각각 3개씩 뽑아라. 반드시 아래 형식대로 한 줄에 하나씩, 번호만 붙여서.
...
[길이] ★띄어쓰기 포함 10자 이내★ 무조건. 짧을수록 임팩트. 길면 탈락.
   ✗ "하루만 필요한데 에어컨 사는 거 진짜예요?" (너무 김)
   ✓ "하루 쓰고 살 거예요?" (10자)  ✓ "여름 땀 빼는 법" (8자)

[끝맺음] 문장을 딱 끝내라. "~법", "~이유", "~꿀팁", "~3가지"처럼 명사로 끝나면 강하다.
   ✗ "에어컨 안 사는 이유" 보다 ✓ "에어컨 안 사는 법" / ✗ "~없잖아요" 같은 흐린 끝맺음 금지.

[표본 좁힌 버전] 그 업종의 구체적 상황·고민을 콕. 살 사람한테 정확히 박히게.

[표본 넓힌 버전] ★개념을 한 단계 위로 올려라★
   제품 자체가 아니라, 그 제품이 속한 '더 큰 욕망/카테고리'로 치환해야 진짜 넓어진다.
   · 이동식에어컨 → (상위개념: 더위) → "40도에 일하는 법", "한여름 땀 안 흘리는 법"
   · 불닭 → (상위개념: 다이어트·매운맛) → "다이어트 망치는 이것", "한국인 매운맛 끝판"
   · 소프트스톤 → (상위개념: 집값·고급 인테리어) → "집값 올리는 자재"
   ※ "에어컨 안 사는 이유"는 넓힌 게 아니다(여전히 에어컨 얘기). 반드시 '에어컨→더위'처럼 상위로 올려라.
   ※ 단, 제품과 연관은 유지. 너무 동떨어지면 안 됨.

[반복 금지] 이전에 나온 카피(예: "공기관 500곳", "단 하루 300만원")를 또 쓰지 마라. 매번 완전히 새로.

[금지 표현] "싼 게 더 비싸다" 류 억지반전 / "~진실" "~폭로" 남발 / "~없잖아요" 흐린 끝 / "프로필 보세요".`,

    script: `## 【대본】
${isShort
  ? `아래 표 형식으로 작성. 반드시 이 흐름을 지켜라:
   ① 후킹(0~2초) → ② 권위/신뢰 → ③ ★핵심 정보·USP★ → ④ CTA
...
★★중요★★ ③ '핵심 정보·USP' 구간을 반드시 넣어라. 후킹·권위만 있고 제품 설명이 없으면 실패다.
   - 이 제품/서비스가 '구체적으로 뭐고, 뭐가 다른지'를 한두 구간 보여줘라.
   - 차별점·강점(USP 입력값)을 실제 멘트로 풀어라. ("이건 ~해서 ~합니다" 식)
   - 예: 권위 박은 뒤 → "저희는 ○○ 방식이라 △△가 다릅니다. 그래서 ◇◇해요" → 그 다음 CTA.`
  : "구어체 대본을 단락으로. 6단계(①후킹+타깃 ②권위 ③통념깨기 ④★핵심정보·USP(제품이 뭐고 뭐가 다른지 구체적으로)★ ⑤사례+숫자 ⑥CTA)가 보이도록 소제목을 붙여라. ④ USP 설명을 빠뜨리지 마라."}`,
...
## 절대 규칙
1. 제품의 USP(강점)는 반드시 콘텐츠 안에 명확히 담는다. 특히 대본에는 '이 제품이 뭐고 뭐가 다른지' 설명 구간을 꼭 넣어라(후킹·권위만 있고 제품 설명 없으면 실패).
