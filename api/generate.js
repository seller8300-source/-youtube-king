...
function buildPrompt({ biz, persona, format, section }) {
  ...
  const SEC = {
    copy: `## 【${copyLabel}】
> 📈 표본 넓히기: "(좁은 증상/제품)" → "(큰 욕망)" → "(최상의 이득)" 으로 끌어올렸습니다.

**후보 1.** \`카피\`
**후보 2.** \`카피\`
**후보 3.** \`카피\`

[${copyLabel} 핵심 규칙 — 반드시 지켜라]
- ${isShort ? "12자 이내로 짧고 강하게. 영상 위에 박는 한 줄." : "클릭을 부르는 섬네일 문구. 15자 이내. 궁금증·충격으로 '클릭 전에' 낚아야 한다."}

★★ 표본 넓히기 + 최상의 이득 — 생성 전에 반드시 이 3단계를 거쳐라 ★★
1단계: 이 제품/서비스를 찾는 좁은 증상·제품명이 무엇인지 파악한다. (예: "오다리 교정", "소프트스톤")
2단계: 그걸 그대로 쓰지 말고, 그 증상이 풀렸을 때 사람들이 진짜 원하는 '더 큰 욕망'으로 치환한다.
3단계: ★가장 중요★ 그 욕망이 충족된 '최상의 이득(최종 결과 장면)'을 카피로 제시한다.
   고객은 '수단'이 아니라 '결과'를 산다. 드릴이 아니라 구멍을, 구멍이 아니라 완성된 멋진 선반을 산다.
   - 오다리 교정(수단) → 다리 길어 보이기(욕망) → "치마가 예뻐지는 다리 라인"(최상의 이득)
   - 소프트스톤(수단) → 고급 인테리어(욕망) → "손님이 감탄하는 매장"(최상의 이득)

[금지] 증상명·진단명·제품명을 그대로 쓰지 마라.

[3개 후보는 서로 다른 이득 각도로]
   · 1번: 외모/매력·결과 이득   · 2번: 비밀/고수 각도   · 3번: 손실 회피(최상의 이득을 놓치는 손해)`,
